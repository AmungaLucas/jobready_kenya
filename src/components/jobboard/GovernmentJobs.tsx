    ...
    <section className="section-bg py-10 border-t border-gray-200/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-6">
          <div>
            <h2 className="text-xl sm:text-2xl font-extrabold text-gray-800">Public Sector Opportunities</h2>
            <p className="text-sm text-gray-500 font-light">
              National and county government vacancies, internships, shortlisting updates, and recruitment notices.
            </p>
          </div>
          <Link href="/government-jobs" className="text-sm font-semibold text-emerald-600 hover:text-emerald-700 transition">
            View All →
          </Link>
        </div>
            ...
            <ul className="divide-y divide-gray-200/50 px-4">
              {nationalJobs.map((job) => (
                <li key={job.slug} className="gov-item py-3 flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <Link href={`/jobs/${job.slug}`} className="gov-title text-sm font-semibold text-gray-800 transition hover:text-emerald-600 block truncate">{job.title}</Link>
                    <span className="gov-org text-xs text-gray-400 transition">{job.organization?.orgName} · National</span>
                  </div>
            ...
            <ul className="divide-y divide-gray-200/50 px-4">
              {countyJobs.map((job) => (
                <li key={job.slug} className="gov-item py-3 flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <Link href={`/jobs/${job.slug}`} className="gov-title text-sm font-semibold text-gray-800 transition hover:text-emerald-600 block truncate">{job.title}</Link>
                    <span className="gov-org text-xs text-gray-400 transition">{job.organization?.orgName || 'County Gov.'} · {job.locationCounty}</span>
                  </div>
                  <span className="text-xs text-gray-300">Closes {formatDate(job.deadline)}</span>
                </li>
              ))}
